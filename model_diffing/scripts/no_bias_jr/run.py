...
from model_diffing.models.initialization.no_bias_jr_init import NoBiasJumpReLUInitStrategy
...
def build_trainer(cfg: NoBiasJanUpdateExperimentConfig, bias: bool) -> JanUpdateCrosscoderTrainer:
    device = get_device()
    ...
    crosscoder = AcausalCrosscoder(
        ...
        init_strategy=NoBiasJumpReLUInitStrategy(
            dec_init_norm=0.1,
    ...
    if not bias:
        crosscoder.b_dec_XD.requires_grad_(False)
        crosscoder.b_enc_H.requires_grad_(False)
        crosscoder.b_dec_XD.zero_()
        crosscoder.b_enc_H.zero_()
...
if __name__ == "__main__":
    ...
    fire.Fire(run_exp(lambda cfg: build_trainer(cfg, bias=False), NoBiasJanUpdateExperimentConfig))
    fire.Fire(run_exp(lambda cfg: build_trainer(cfg, bias=True), NoBiasJanUpdateExperimentConfig))
