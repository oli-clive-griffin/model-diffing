...
from model_diffing.trainers.train_l1_crosscoder.config import L1TrainConfig
from model_diffing.trainers.utils import get_l0_stats
from model_diffing.utils import calculate_reconstruction_loss_summed_norm_MSEs, get_fvu_dict, l1_norm, l2_norm
...
class L1CrosscoderTrainer(BaseAcausalTrainer[L1TrainConfig, ReLUActivation]):
    def _calculate_loss_and_log(
        ...
        sparsity_loss = sparsity_loss_l1_of_l2s(self.crosscoder.W_dec_LXD, train_res.latents_BL)
...
def sparsity_loss_l1_of_l2s(
    ...
    norms_LX = einops.reduce(W_dec_LXD, "... d_model -> ...", l2_norm)
    # norms_LX_ = einx.reduce("... [d_model]", W_dec_LXD, l2_norm)
