...
from model_diffing.trainers.train_topk_crosscoder.config import TopKTrainConfig
from model_diffing.utils import calculate_reconstruction_loss_summed_norm_MSEs, get_fvu_dict, not_none
...
class TopKStyleTrainer(BaseAcausalTrainer[TopKTrainConfig, BatchTopkActivation | GroupMaxActivation | TopkActivation]):
    def _calculate_loss_and_log(
        ...
        aux_loss = self.aux_loss(batch_BXD, train_res)
    ...
    def aux_loss(self, batch_BXD: torch.Tensor, train_res: AcausalCrosscoder.ForwardResult) -> torch.Tensor:
        ...
        return aux_loss(
            ...
            decode_BXD=self.crosscoder.decode_BXD,
            error_BXD=batch_BXD - train_res.recon_acts_BXD,
        )
